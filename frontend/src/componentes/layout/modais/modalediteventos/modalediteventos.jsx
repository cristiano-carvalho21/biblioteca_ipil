...
function ModalEditEventos({eventos, onClose, setEventos})
{
  ...
  useEffect(() => {
    if (eventos?.id) {
      ...
      api.get(`/livros/gestao-eventos/${eventos.id}/`)
        .then(res => {
          setForm(res.data);
          setLoading(false);
        })
    ...
    try {
      const response = await api.put(`/livros/gestao-eventos/${eventos.id}/`,
        form
      );
      setModal({
          open: true,
      ...
      setEventos(prev =>
        prev.map(ev => (ev.id === eventos.id ? response.data : ev))
      );
